---
title: "Predicting future ill health using deep learning: Model 4"
...
## Model 4: Predicting health status 5 years after baseline for individuals who had a good health at baseline

We now repeat the third model but only for individuals who reported that they did not have a LLTI at baseline. The outcome variable we will look at is whether an individual reported that they had a longstanding illness or disability (yes (1) or no (0)) 5 years after baseline only for individuals without a LLTI (1) at baseline. 
...
table(model4_data$health_5yr) # Print outcome data to look at our outcome variable
...
A greater number of variables were excluded than compared to model 3 leaving a total of **43** variables left.
...
# model4_per %>% fit(dm_train, y, epochs = 10, batch_size = 128)
...
lrfit_model4 <- cbind(lrfit_model4, hold) # join to main table
write.csv(lrfit_model4, "./Model fit statistics/lrfit_model4.csv") # Save
lrfit_model4 # Print
...
rownames(hold)[19] <- "AUC" # Edit row name
hold <- round(hold, digits=4) # Round numbers off for presentation
nnfit_model4 <- cbind(nnfit_model4, hold) # join to main table
write.csv(nnfit_model4, "./Model fit statistics/nnfit_model4.csv") # Save
nnfit_model4 # Print
...
rownames(hold)[19] <- "AUC" # Edit row name
hold <- round(hold, digits=4) # Round numbers off for presentation
xgbfit_model4 <- hold # join to main table
write.csv(xgbfit_model4, "./Model fit statistics/xgbfit_model4.csv") # Save
xgbfit_model4 # Print
...
rownames(hold)[19] <- "AUC" # Edit row name
hold <- round(hold, digits=4) # Round numbers off for presentation
xgbfit_model4 <- cbind(xgbfit_model4, hold) # join to main table
write.csv(xgbfit_model4, "./Model fit statistics/xgbfit_model4.csv") # Save
xgbfit_model4 # Print
...
# Feature importance
mat <- xgb.importance (feature_names = colnames(train_all[,lasso_all_vars,with=F]),model = xgb4)
xgb.plot.importance (importance_matrix = mat[1:20])
# Gain is the improvement in accuracy brought by a feature to the branches it is on.
write.csv(mat, "./Model fit statistics/xgb_featureimport_model4.csv") # Save
