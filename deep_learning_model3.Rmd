---
title: "Predicting future ill health using deep learning: Model 3"
output: html_notebook
author: Mark A Green
---

Ok, let's try a variable with a longer time difference. First, let's set up R and load in the data.
...
## Model 3: Predicting health status 5 years after baseline

We now repeat the first model but instead use a longer outcome. This is important since health conditions take time to materialise and our models so far are focusing on acute changes. The outcome variable we will look at is whether an individual reported that they had a longstanding illness or disability (yes (1) or no (0)) 5 years after baseline. 
...
table(model3_data$health_5yr) # Print outcome data to look at our outcome variable

```

There is a fairly even split on the outcome variable here (43% had a LLTI).
...
#saveRDS(train_split, file = "./Data split files/train_split_health5yr_model3.rds") # Save
...
The LASSO has cut the number of variables by quite a lot here - from 73 to **41**. We can summarise the results by sub-domain:

* Housing (6 to 4) - Household size, number of beds, whether they own their house and the value of the house remain.
* Education (1 to 1) - One variable (highest education qualification) and it remains
* Income (17 to 11) - If they had problems paying for their housing, most of the benefits measures (bar pension and housing/council tax), subjective financial status, gross income, if contributes to a private pension and if they save.
* Occupation (21 to 10) - If their job was permanent, the social class of that job, if they are a manager, how much overtime they worked, if they worked weekends, if they had a second job, and if feel dpressed or gloomy about their job all remain.
* Parental (6 to 3) - Mothers education, if the mother was employed when aged 14 and the fathers occupation aged 14.
* Relative poverty (16 to 7) - Whether individuals had the following remained: a tumble drier, a car or mobile phone to use. Also kept was whether they could afford holiday, house, social meal/drink and contents insurance.
* Social capital (6 to 5) - All bar their level of interest in politics were kept. 
...
A greater number of variables were excluded than compared to model 1 leaving a total of **45** variables left. 
####Biomarkers
...
Testosterone (only for males) was dropped again as was urea (kidney). Other variables dropped were ferritin and Aspartate transaminase. This has left a total of **16** variables.
...
```{r}
source("./load_feature_selected_model3.R") # If have not run all previous lines, have saved the list of variables so call that
...
logit_train_per <- train(as.factor(health_5yr) ~ ., # Formula (the '.' means all columns in the data not stated in the formula)
                         data = temp, 
...
cm <- confusionMatrix(as.factor(test.probs), as.factor(test_labels$health_5yr), positive = "1") # Order should be predictions, then reference/observed

# Store results in clean format
lrfit_model3 <- as.data.frame(as.matrix(cm, what = "overall")) # Accuracy
lrfit_model3 <- rbind(lrfit_model3, as.data.frame(as.matrix(cm, what = "classes"))) # Other metrics
colnames(lrfit_model3) <- c("Personal") # Rename column 

# Calcuate AUC
library(ROCR)
pr <- prediction(as.numeric(test.probs), test_labels$health_5yr) # calcuate predictions again!
auc <- performance(pr, measure = "auc") # Calculate AUC
auc <- auc@y.values[[1]] # Extract only AUC value 
lrfit_model3[19,1] <- auc # Join onto table of results
rownames(lrfit_model3)[19] <- "AUC" # Edit row name
lrfit_model3 <- round(lrfit_model3, digits=4) # Round numbers off for presentation
write.csv(lrfit_model3, "./Model fit statistics/lrfit_model3.csv") # Save
lrfit_model3 # Print
...
build_model <- function() {
  ...
  model3_per <- keras_model_sequential() %>% # State is sequential model (i.e. no reinforced learning)
...
for (i in 1:k) {
  ...
  model3_per <- build_model()
  
  # Train the model (in silent mode: verbose=0)
  model3_per %>% fit(partial_train_inputs, partial_train_labels,
                epochs = num_epochs, batch_size = 128, verbose = 0)
                
  # Evaluate the model on the validation data
  results <- model3_per %>% evaluate(val_inputs, val_labels, verbose = 0)
...
# model3_per %>% fit(dm_train, y, epochs = 10, batch_size = 128)
...
dm_test <- as.matrix(test_data[,lasso_personal_vars,with=F])
test_y <- test_labels$health_5yr 
metrics <- model3_per %>% evaluate(dm_test, test_y, batch_size = 128, verbose = 0)
metrics
...
```{r}
# Predict outcomes
pred_y <- model3_per %>% predict(dm_test, batch_size=128, verbose=0)
...
# Store results in clean format
nnfit_model3 <- as.data.frame(as.matrix(cm2, what = "overall")) # Accuracy
nnfit_model3 <- rbind(nnfit_model3, as.data.frame(as.matrix(cm2, what = "classes"))) # Other metrics
colnames(nnfit_model3) <- c("Personal") # Rename column 
...
auc <- performance(pr, measure = "auc") # Calculate AUC
auc <- auc@y.values[[1]] # Extract only AUC value 
nnfit_model3[19,1] <- auc # Join onto table of results
rownames(nnfit_model3)[19] <- "AUC" # Edit row name
nnfit_model3 <- round(nnfit_model3, digits=4) # Round numbers off for presentation
write.csv(nnfit_model3, "./Model fit statistics/nnfit_model3.csv") # Save
nnfit_model3 # Print
...
logit_train_ses <- train(as.factor(health_5yr) ~ ., # Formula (the '.' means all columns in the data not stated in the formula)
                         data = temp, 
                         trControl = train_control, method = "glm", family = binomial(link = "logit")) 
...
lrfit_model3 <- cbind(lrfit_model3, hold) # join to main table
write.csv(lrfit_model3, "./Model fit statistics/lrfit_model3.csv") # Save
lrfit_model3 # Print
...
build_model <- function() {
  ...
  # Define network structure
  model3_ses <- keras_model_sequential() %>% # State is sequential model (i.e. no reinforced learning)
...
for (i in 1:k) {
  ...
  # Build the Keras model (already compiled previously)
  model3_ses <- build_model()
  
  # Train the model (in silent mode: verbose=0)
  model3_ses %>% fit(partial_train_inputs, partial_train_labels,
                epochs = num_epochs, batch_size = 128, verbose = 0)
                
  # Evaluate the model on the validation data
  results <- model3_ses %>% evaluate(val_inputs, val_labels, verbose = 0)
...
test_y <- test_labels$health_5yr 
metrics <- model3_ses %>% evaluate(dm_test, test_y, batch_size = 128, verbose = 0)
metrics
...
```{r}
# Predict outcomes
pred_y <- model3_ses %>% predict(dm_test, batch_size=128, verbose=0)
pred.nn <- rep(0,length(pred_y))
pred.nn[pred_y>=0.5] <- 1
cm2 <- confusionMatrix(as.factor(pred.nn), as.factor(test_y), positive = "1")  # Calculate model fit measures
...
rownames(hold)[19] <- "AUC" # Edit row name
hold <- round(hold, digits=4) # Round numbers off for presentation
nnfit_model3 <- cbind(nnfit_model3, hold) # join to main table
write.csv(nnfit_model3, "./Model fit statistics/nnfit_model3.csv") # Save
nnfit_model3 # Print
...
logit_train_health <- train(as.factor(health_5yr) ~ ., # Formula (the '.' means all columns in the data not stated in the formula)
                         data = temp, 
                         trControl = train_control, method = "glm", family = binomial(link = "logit")) 
...
build_model <- function() {
  ...
  # Define network structure
  model3_health <- keras_model_sequential() %>% # State is sequential model (i.e. no reinforced learning)
    layer_dense(units = 64, activation = "relu", kernel_regularizer = regularizer_l1(0.001)) %>% # First layer. We include l1 regularisation to minimise potential for overfitting, although its affect is small. L1 regularisation was selected merely because it is the same as the LASSO regression.
    layer_dense(units = 1, activation = "sigmoid") # Output layer - generate a predicted prob

  model3_health %>% compile(
    optimizer = "rmsprop", # Optimiser - how the network will update itself during the training phase - in our case we will use Gradient Descent since has been shown to be most effective and efficient (this is one algorithm for it)
    loss = "binary_crossentropy", # Best choice for dealing with probabilities and binary labels. Crossentropy loss function measures the difference between the probability and label values.
    metrics = c("precision") # Evaluate model based on precision 
  )
...
for (i in 1:k) {
  ...
  # Build the Keras model (already compiled previously)
  model3_health <- build_model()
  
  # Train the model (in silent mode: verbose=0)
  model3_health %>% fit(partial_train_inputs, partial_train_labels,
                epochs = num_epochs, batch_size = 128, verbose = 0)
                
  # Evaluate the model on the validation data
  results <- model3_health %>% evaluate(val_inputs, val_labels, verbose = 0)
...
test_y <- test_labels$health_5yr 
metrics <- model3_health %>% evaluate(dm_test, test_y, batch_size = 128, verbose = 0)
metrics
...
```{r}
# Predict outcomes
pred_y <- model3_health %>% predict(dm_test, batch_size=128, verbose=0)
pred.nn <- rep(0,length(pred_y))
pred.nn[pred_y>=0.5] <- 1
cm2 <- confusionMatrix(as.factor(pred.nn), as.factor(test_y), positive = "1")  # Calculate model fit measures
...
logit_train_biom <- train(as.factor(health_5yr) ~ ., # Formula (the '.' means all columns in the data not stated in the formula)
                         data = temp, 
                         trControl = train_control, method = "glm", family = binomial(link = "logit")) 
...
build_model <- function() {
  ...
  # Define network structure
  model3_biom <- keras_model_sequential() %>% # State is sequential model (i.e. no reinforced learning)
    layer_dense(units = 64, activation = "relu", kernel_regularizer = regularizer_l1(0.001)) %>% # First layer. We include l1 regularisation to minimise potential for overfitting, although its affect is small. L1 regularisation was selected merely because it is the same as the LASSO regression.
    layer_dense(units = 1, activation = "sigmoid") # Output layer - generate a predicted prob
...
for (i in 1:k) {
  ...
  # Build the Keras model (already compiled previously)
  model3_biom <- build_model()
  
  # Train the model (in silent mode: verbose=0)
  model3_biom %>% fit(partial_train_inputs, partial_train_labels,
                epochs = num_epochs, batch_size = 128, verbose = 0)
                
  # Evaluate the model on the validation data
  results <- model3_biom %>% evaluate(val_inputs, val_labels, verbose = 0)
...
test_y <- test_labels$health_5yr 
metrics <- model3_biom %>% evaluate(dm_test, test_y, batch_size = 128, verbose = 0)
metrics
...
```{r}
# Predict outcomes
pred_y <- model3_biom %>% predict(dm_test, batch_size=128, verbose=0)
pred.nn <- rep(0,length(pred_y))
pred.nn[pred_y>=0.5] <- 1
cm2 <- confusionMatrix(as.factor(pred.nn), as.factor(test_y), positive = "1")  # Calculate model fit measures
...
logit_train_gen <- train(as.factor(health_5yr) ~ ., # Formula (the '.' means all columns in the data not stated in the formula)
                         data = temp, 
                         trControl = train_control, method = "glm", family = binomial(link = "logit")) 
...
build_model <- function() {
  ...
  # Define network structure
  model3_gen <- keras_model_sequential() %>% # State is sequential model (i.e. no reinforced learning)
...
for (i in 1:k) {
  ...
  # Build the Keras model (already compiled previously)
  model3_gen <- build_model()
  
  # Train the model (in silent mode: verbose=0)
  model3_gen %>% fit(partial_train_inputs, partial_train_labels,
                epochs = num_epochs, batch_size = 128, verbose = 0)
                
  # Evaluate the model on the validation data
  results <- model3_gen %>% evaluate(val_inputs, val_labels, verbose = 0)
...
test_y <- test_labels$health_5yr 
metrics <- model3_gen %>% evaluate(dm_test, test_y, batch_size = 128, verbose = 0)
metrics
...
```{r}
# Predict outcomes
pred_y <- model3_gen %>% predict(dm_test, batch_size=128, verbose=0)
pred.nn <- rep(0,length(pred_y))
pred.nn[pred_y>=0.5] <- 1
cm2 <- confusionMatrix(as.factor(pred.nn), as.factor(test_y), positive = "1")  # Calculate model fit measures
...
rownames(hold)[19] <- "AUC" # Edit row name
hold <- round(hold, digits=4) # Round numbers off for presentation
lrfit_model3 <- cbind(lrfit_model3, hold) # Join to original table
rm(hold)
...
ggsave("./Plots/corr_plot_model3_paper.tiff", cor_plot, device = "tiff", dpi = 300)
ggsave("./Plots/corr_plot_model3_prez.png", cor_plot, device = "png", dpi = 300)
print(cor_plot)

```

Somewhat similar to model3 at the positive end of the distribution. At the negative end we see walking pace, job security, if the job was permanent and the physicality of work. There are a lot more negative predictors again - what are helping us to predict 0s which is probably less useful as we really want to predict the 1s.

```{r}
library(ICEbox)
ice_hcond96 <- ice(object = model3_all, X = dm_train, # You can also do for test data
...
ice_age <- ice(object = model3_all, X = dm_train, 
               predictor = "age", frac_to_build = 1)
ice_hba1c <- ice(object = model3_all, X = dm_train,  
               predictor = "hba1c", frac_to_build = 1)
ice_marstat <- ice(object = model3_all, X = dm_train, 
               predictor = "marstat", frac_to_build = 1)

#png("./Plots/ice_plot_model3_positive.png")
par(mfrow=c(2,2))
...
plot(ice_hba1c, frac_to_plot = 0.05, xlab = "Standardised Values", ylab = "Partial Predicted Outcome", plot_pdp = T, main = "HbA1c")
#dev.off()
...
```{r}
#png("./Plots/ice_plot_model3_positive_c.png")
par(mfrow=c(2,2))
...
plot(ice_hba1c, frac_to_plot = 0.05, xlab = "Standardised Values", ylab = "Partial Predicted Outcome", plot_pdp = T, main = "HbA1c", centered = T)
#dev.off()
...
```{r}
ice_jbperm <- ice(object = model3_all, X = dm_train, 
               predictor = "jbperm", frac_to_build = 1)
ice_wkphys <- ice(object = model3_all, X = dm_train,
               predictor = "b_wkphys", frac_to_build = 1)
ice_walkpace <- ice(object = model3_all, X = dm_train, 
               predictor = "b_walkpace", frac_to_build = 1)
ice_jbsec <- ice(object = model3_all, X = dm_train,  
               predictor = "b_jbsec", frac_to_build = 1)

#png("./Plots/ice_plot_model3_negative.png")
par(mfrow=c(2,2))
...
plot(ice_walkpace, frac_to_plot = 0.05, xlab = "Standardised Values", ylab = "Partial Predicted Outcome", plot_pdp = T, main = "Walking pace")
...
```{r}
#png("./Plots/ice_plot_model3_negative_c.png")
par(mfrow=c(2,2))
...
plot(ice_walkpace, frac_to_plot = 0.05, xlab = "Standardised Values", ylab = "Partial Predicted Outcome", plot_pdp = T, main = "Walking pace", centered = T)
...
rownames(hold)[19] <- "AUC" # Edit row name
hold <- round(hold, digits=4) # Round numbers off for presentation
xgbfit_model3 <- hold # join to main table
write.csv(xgbfit_model3, "./Model fit statistics/xgbfit_model3.csv") # Save
xgbfit_model3 # Print
...
rownames(hold)[19] <- "AUC" # Edit row name
hold <- round(hold, digits=4) # Round numbers off for presentation
xgbfit_model3 <- cbind(xgbfit_model3, hold) # join to main table
write.csv(xgbfit_model3, "./Model fit statistics/xgbfit_model3.csv") # Save
xgbfit_model3 # Print
...
# Feature importance
mat <- xgb.importance (feature_names = colnames(train_all[,lasso_all_vars,with=F]),model = xgb3)
xgb.plot.importance (importance_matrix = mat[1:20])
# Gain is the improvement in accuracy brought by a feature to the branches it is on.
write.csv(mat, "./Model fit statistics/xgb_featureimport_model3.csv") # Save
